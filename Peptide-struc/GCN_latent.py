...
parser.add_argument('--batch_size', type=int, default=15, help='Latent Dimension')
...
class HNO(torch.nn.Module):
    ...
    def __init__(self,):
        ...
        self.conv1 = GCNConv(total_feats, int(2*args.hidden))#,K=6)
        ...
        self.conv2 = GCNConv(int(2*args.hidden), int(2*args.hidden))#,K=6)
        self.conv3 = GCNConv(int(2*args.hidden), args.hidden)#,K=6)
        self.conv4 = GCNConv(int(args.hidden), args.hidden)#,K=6)
...
wandb.init(
project="Final_ICML_PeptideStruc",
name="4GAT",
config=config,
)
  ...
  if val_loss<temp:
    ...
    torch.save({
        ...
        'best_weights':weights,
          }, './struc_weights/'+args.webdata+'_'+str(epoch)+'.pth')
...
device="cuda"
checkpoint = torch.load('./struc_weights/'+args.webdata+'_'+str(when)+'.pth')
