...
parser.add_argument('--seed', type=int, default=67, help='Latent Dimension')
parser.add_argument('--batch_size', type=int, default=20, help='Latent Dimension')
...
parser.add_argument('--webdata', type=str, default='wdstfcyv', help='which data')
...
wandb.init(
project="ICML_2ways_attention_Rewiring",
name="GCN_backbone_AdamW",
config=config,
)
...
for epoch in range(250):
  ...
  for i, data in enumerate(trainloader):
    ...
    if args.pretrain==True:
      ...
      classify,atts, new_edges =model(feats,data.edge_index,data.batch,weights,data.batch.unique().shape[0],device,data,pretrain=args.pretrain)
  ...
  for j, valdata in enumerate(valoader):
    ...
    if args.pretrain==True:
      ...
      val_classify,val_atts,val_highest_weight_nodes =model(valfeats,valdata.edge_index,valdata.batch,weights,valdata.batch.unique().shape[0],device,valdata,pretrain=args.pretrain)
  ...
  if val_perf>=temp:
    ...
    torch.save({
        ...
        'best_weights':weights,
          }, './rewire_weights/'+args.webdata+'_'+str(epoch)+'.pth')
...
device="cuda"
checkpoint = torch.load('./rewire_weights/'+args.webdata+'_'+str(when)+'.pth')
